    ...
    stages {
       ...
       /* stage('Maven Build') {
        ...
        stage('Build Docker Image') {
            steps {
                script {
                 ...
                 sh 'docker build -t maheshreddy123/flask:v6 .'
               ...
               bat 'docker push maheshreddy123/flask:v2'
                    ...
                    withCredentials([string(credentialsId: 'dockerhup', variable: 'dockerhp')]) {
                        sh "docker login -u maheshreddy123 -p ${dockerhp}"
                            ...
                            sh 'docker push maheshreddy123/flask:v6'     
              }
            }
          }
        stage('Run Container on server1'){
            steps {
                sshagent(['ser1']) {
                sh "ssh -o StrictHostKeyChecking=no ec2-user@65.2.150.76 'docker run -p 4000:4000 -d maheshreddy123/falsk:v6'"
                }
